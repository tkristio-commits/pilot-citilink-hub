...
- `model` (required) — for Nano Banana stills, default **`nano-banana-2`**; optional **`nano-banana`** (Nano Banana Pro). Other image engines: `gpt-image` (older), `gpt-image-2` (ChatGPT Image 2 — 32 credits), `soul`, `grok_image`, `seedream`, `seedream_5_lite`
